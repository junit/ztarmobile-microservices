 ...
 */
package com.ztarmobile.transaction.notification;
...
public class CustomerUsageMailSender {
     ...
     */
    @Async
    public void sendEmail(TransactionsEmailNotification email) {
     ...
     */
    private void createMessageBody(TransactionsEmailNotification email) {
        if (email.getMessageBody() == null) {
            // creates the body based on a template.
            final Context ctx = new Context();
            ctx.setVariable("vendor", email.getVendor());
            ctx.setVariable("month", email.getMonth());

            final String htmlContent = this.templateEngine.process("cc_transactions", ctx);
